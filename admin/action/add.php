		...
		function getCategoryListTable() {
			...
			if ($result) {
				for ($i = 0; $i < $result[0]; $i++) {
					...
					if ($res[$i]['levelup'] == "1") {
						$cats_list .= $this->replaceCategoryRowValues($res[$i]['ID'], $res[$i]['title'], $res[$i]['name'], "");
						...
						if ($result2) {
							...
							for ($j = 0; $j < $result2[0]; $j++) {
								...
								$cats_list .= $this->replaceCategoryRowValues($res2[$j]['ID'], $res2[$j]['title'], $res2[$j]['name_1']."/".$res2[$j]['name'], " -- ");
							}
						}
					}
				}
